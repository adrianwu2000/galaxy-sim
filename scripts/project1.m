...
v(ncore + 1:N + ncore, 2) = v1 .* sin(theta1 - pi/2) - 0;
% core2
...
for step = 2:nt-1
    if plotenable
      ...
      % Draw the ball.
      plot(r(1,1,step), r(1, 2, step), 'Marker', ballmarker, 'MarkerSize', 10, ...
         'MarkerEdgeColor', 'c', 'MarkerFaceColor', 'c')
      plot(r(2,1,step), r(2, 2, step), 'Marker', ballmarker, 'MarkerSize', 10, ...
    ...
    r_current = r(:, :, step);
    r_prev = r(:, :, step-1);
    r_next = nbodyupdate(m, r_current, r_prev, dt);
    r(:, :, step+1) = r_next;
end
